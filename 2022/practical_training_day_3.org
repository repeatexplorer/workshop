...
fastqc *.fastq.gz
#+end_src
When QC run finishe, inspect html reports.

**  Filtering on quality
Use *Trimmomatic* program to remove low quality reads and trim ends. Trimmomatic will be used in Pair-end mode. Trimmomatic options are:
#+begin_src text
